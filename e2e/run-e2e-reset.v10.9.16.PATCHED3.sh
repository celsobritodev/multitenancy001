...
# =========================================================
# Verifica se a porta ja esta em uso (com cores)
...
check_port_available() {
  local port="$1"
  banner "Verificando se a porta $port esta disponivel"
  ...
  if command -v netstat >/dev/null 2>&1; then
    ...
    echo -e "${YELLOW}⚠️  Nao foi possivel verificar se a porta $port esta em uso (nenhuma ferramenta encontrada). Continuando mesmo assim...${NC}"
    return 0
  fi

  if $port_in_use; then
    echo -e "${RED}❌ Porta $port ja esta em uso! (verificado com: $check_method)${NC}"
    echo -e "${YELLOW}   Encerre o processo que esta usando a porta antes de executar os testes E2E.${NC}"
    echo ""
    ...
    echo -e "${GREEN}✅ Porta $port disponivel. (verificado com: $check_method)${NC}"
    return 0
  fi
}
#!/bin/bash
