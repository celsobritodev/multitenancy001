...
LOG_FILE="${LOG_DIR}/execucao_v20_${TIMESTAMP}.log"
APP_LOG="${LOG_DIR}/app_${TIMESTAMP}.log"
REPORT_DIR="${LOG_DIR}/reports_${TIMESTAMP}"
mkdir -p "${LOG_DIR}" "${REPORT_DIR}"

COLLECTION="${SCRIPT_DIR}/multitenancy001.postman_collection.v20.0.chaos-race-enterprise.strict.json"
ENV_FILE="${SCRIPT_DIR}/multitenancy001.local.postman_environment.v20.0.chaos-race-enterprise.strict.json"
...
echo "────────────────────────────────────────────────────"
echo "🔷 TESTE V20.0 - CHAOS RACE ENTERPRISE (ULTRA SUITE)"
...
title "Executando suíte base V20"
...
: > "${CHAOS_RACE_REPORT}"
statuses_file="${REPORT_DIR}/chaos-race-statuses.txt"
: > "${statuses_file}"
...
for i in $(seq 1 "${ATTEMPTS}"); do
  (
    code="$("${SCRIPT_DIR}/chaos-race-worker-sale.sh" "${BASE_URL_EFFECTIVE}" "${TOKEN}" "${SCHEMA}" "${CUSTOMER_ID}" "${PRODUCT_ID}" "${UNIT_PRICE}" "${PRODUCT_NAME}" "${MAX_JITTER_MS}" "${BASE_DELAY_MS}" "${RETRY_COUNT}")"
...
echo "attempts=${ATTEMPTS}" > "${CHAOS_RACE_REPORT}"
echo "parallelism=${PARALLELISM}" >> "${CHAOS_RACE_REPORT}"
echo "max_jitter_ms=${MAX_JITTER_MS}" >> "${CHAOS_RACE_REPORT}"
echo "base_delay_ms=${BASE_DELAY_MS}" >> "${CHAOS_RACE_REPORT}"
echo "retry_count=${RETRY_COUNT}" >> "${CHAOS_RACE_REPORT}"
echo "success_count=${success_count}" >> "${CHAOS_RACE_REPORT}"
echo "failure_count=${failure_count}" >> "${CHAOS_RACE_REPORT}"
echo "rollback_like_count=${rollback_like_count}" >> "${CHAOS_RACE_REPORT}"
...
newman run "${COLLECTION}" -e "${TEMP_ENV}" --folder "📦 92 - CHAOS RACE VERIFY" --reporters cli,json --reporter-json-export "${REPORT_DIR}/chaos-race-verify.json"
ok "Verificação pós-chaos race concluída"
hr

title "Executando rollback probe"
newman run "${COLLECTION}" -e "${TEMP_ENV}" --folder "📦 93 - CHAOS ROLLBACK PROBE" --reporters cli,json --reporter-json-export "${REPORT_DIR}/chaos-rollback-probe.json"
...
ok "TESTE V20.0 ULTRA FINALIZADO"
