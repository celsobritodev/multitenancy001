...
COLLECTION="${SCRIPT_DIR}/multitenancy001.postman_collection.v32.2.hardening-real-stabilized.json"
ENV_FILE="${SCRIPT_DIR}/multitenancy001.local.postman_environment.v32.2.hardening-real-stabilized.json"
...
[[ -x "${WORKER_SCRIPT}" ]] || { err "Worker script não encontrado ou não executável: ${WORKER_SCRIPT}"; exit 1; }
[[ -x "${AGGREGATOR_SCRIPT}" ]] || { err "Aggregator script não encontrado ou não executável: ${AGGREGATOR_SCRIPT}"; exit 1; }
...
V32_ULTRA_CYCLES="${V32_ULTRA_CYCLES:-3}"
jq --arg V32MODE "ULTRA" --arg V32CYCLES "${V32_ULTRA_CYCLES}" '
  .values = (
    [.values[] | if .key=="v32_hardening_mode" then .value=$V32MODE
                 elif .key=="v32_ultra_cycles" then .value=$V32CYCLES
                 else . end] +
    (if ([.values[] | select(.key=="v32_hardening_mode")] | length)==0 then [{"key":"v32_hardening_mode","value":$V32MODE,"enabled":true}] else [] end) +
    (if ([.values[] | select(.key=="v32_ultra_cycles")] | length)==0 then [{"key":"v32_ultra_cycles","value":$V32CYCLES,"enabled":true}] else [] end)
  )' "${TEMP_ENV}" > "${TEMP_ENV}.tmp" && mv "${TEMP_ENV}.tmp" "${TEMP_ENV}"
step "Modo hardening: ULTRA | cycles=${V32_ULTRA_CYCLES}"
...
step "Impacta inventory: ${SALE_AFFECTS_INVENTORY}"
...
[[ -n "${agg_warnings}" ]] && warn "Warnings do aggregator: ${agg_warnings}"
hr
...
(( actual_consumption > inventory_before_real_race )) && { err "Oversell real detectado: consumo real (${actual_consumption}) excede o saldo inicial (${inventory_before_real_race})"; update_env_value final_suite_result FAILED_OVERSELL_CONSUMPTION_EXCEEDED; exit 1; }
