...
#   - Reset database
#   - Start application
#   - Run patched enterprise collection
#   - Create 10 customers
#   - Create 10 sales linked explicitly to real customerId values
#   - Preserve warnings, icons, and visual feedback style
#   - Parse real Newman JSON counters for executive summary
#   - Fallback to mvn spring-boot:run if ./mvnw does not exist
# ==============================================================================
...
NEWMAN_JSON=".e2e-${SCRIPT_VERSION}.newman-report.json"
...
out_path = Path(".e2e-v6.9.3.1.enterprise-customers-sales-consolidated.collection.json")
...
for idx, item in enumerate(customer_items[:10], start=1):
    ...
    tests = [
        ...
        f"    pm.collectionVariables.set('customer_id_{idx}', res.id);",
...
run_newman() {
  ...
  echo
  rm -f "${NEWMAN_JSON}"
  newman run "${PATCHED_COLLECTION}"     -e "${EFFECTIVE_ENV}"     -r cli,json     --reporter-json-export "${NEWMAN_JSON}"
}

parse_newman_json() {
  if [[ ! -f "${NEWMAN_JSON}" ]]; then
    print_warn "Newman JSON report not found: ${NEWMAN_JSON}"
    return
  fi

  local parsed
  parsed="$(python3 - "${NEWMAN_JSON}" <<'PY'
import json, sys
from pathlib import Path
...
requests_total = stats.get("requests", {}).get("total", "N/A")
assertions_total = stats.get("assertions", {}).get("total", "N/A")
failures_total = len(failures)

duration_ms = timings.get("completed")
if duration_ms is None:
    duration_ms = timings.get("responseAverage")
if duration_ms is None:
    duration_ms = "N/A"

print(requests_total)
print(assertions_total)
print(failures_total)
print(duration_ms)
PY
)" || true

  if [[ -n "${parsed}" ]]; then
    NEWMAN_REQUESTS="$(echo "${parsed}" | sed -n '1p')"
    NEWMAN_ASSERTIONS="$(echo "${parsed}" | sed -n '2p')"
    NEWMAN_FAILURES="$(echo "${parsed}" | sed -n '3p')"
    NEWMAN_DURATION_MS="$(echo "${parsed}" | sed -n '4p')"
    NEWMAN_DURATION_HUMAN="$(format_duration_ms "${NEWMAN_DURATION_MS}")"
  fi
}
...
print_final_summary() {
  ...
  echo "✅ Newman JSON report: ${NEWMAN_JSON}"
  echo "✅ Environment: ${ENV_FILE}"
  echo "✅ Path: ${pwd_path}"
  echo "✅ Start command: ${APP_START_CMD}"
  ...
  echo "✅ Requests executados: ${NEWMAN_REQUESTS}"
  echo "✅ Assertions executadas: ${NEWMAN_ASSERTIONS}"
  echo "✅ Failures: ${NEWMAN_FAILURES}"
  echo "✅ Tempo total Newman: ${NEWMAN_DURATION_HUMAN} (${NEWMAN_DURATION_MS} ms)"
...
main() {
  require_cmd python3 "Install Python 3 and try again."
  require_cmd curl "Install curl and try again."
  require_cmd psql "Install PostgreSQL client tools and try again."
  require_cmd newman "Install Newman and try again."
