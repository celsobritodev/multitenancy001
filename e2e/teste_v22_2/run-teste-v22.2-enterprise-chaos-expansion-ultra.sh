...
LOG_FILE="${LOG_DIR}/execucao_v22.2_${TIMESTAMP}.log"
APP_LOG="${LOG_DIR}/app_${TIMESTAMP}.log"
REPORT_DIR="${LOG_DIR}/reports_${TIMESTAMP}"
mkdir -p "${LOG_DIR}" "${REPORT_DIR}"

COLLECTION="${SCRIPT_DIR}/multitenancy001.postman_collection.v22.2.enterprise-chaos-expansion.strict.json"
ENV_FILE="${SCRIPT_DIR}/multitenancy001.local.postman_environment.v22.2.enterprise-chaos-expansion.strict.json"
...
fetch_inventory_snapshot() {
  ...
  inventory_json="$(curl -fsS \
    ...
    -H "X-Tenant: ${SCHEMA}" \
    "${BASE_URL_EFFECTIVE}/api/tenant/inventory/products/${product_id}")"

  movements_json="$(curl -fsS \
    -H "Authorization: Bearer ${TOKEN}" \
    -H "X-Tenant: ${SCHEMA}" \
    "${BASE_URL_EFFECTIVE}/api/tenant/inventory/products/${product_id}/movements")"
    ...
    if isinstance(obj, dict):
        for k in ("content", "items", "data", "results"):
            if isinstance(obj.get(k), list):
                return len(obj[k])
        if isinstance(obj.get("totalElements"), int):
...
qty = (
    inv.get("quantityAvailable")
    if isinstance(inv, dict) and inv.get("quantityAvailable") is not None
    else inv.get("quantity")
    if isinstance(inv, dict) and inv.get("quantity") is not None
    else inv.get("availableQuantity")
    if isinstance(inv, dict) and inv.get("availableQuantity") is not None
    else 0
)
mov = json.loads(mov_raw)
print(f"{int(qty)}\t{int(parse_count(mov))}")
PY
}
...
echo "────────────────────────────────────────────────────"
echo "🔷 TESTE V22.2 - ENTERPRISE CHAOS EXPANSION (ULTRA SUITE)"
...
echo "DEBUG_PRE inventory=${inventory_before_real_race} movements=${movements_before_real_race}" >> "${LOG_FILE}"
...
title "DEBUG SNAPSHOT POST-CHAOS (CONFIRMAÇÃO)"
read -r debug_inventory_post debug_movements_post < <(fetch_inventory_snapshot "${PRODUCT_ID}")
step "DEBUG_POST inventory=${debug_inventory_post} movements=${debug_movements_post}"
echo "DEBUG_POST inventory=${debug_inventory_post} movements=${debug_movements_post}" >> "${LOG_FILE}"
...
if (( actual_inventory_after < 0 )); then
  err "Oversell real detectado: inventory final negativo (${actual_inventory_after})"
  update_env_value "ledger_race_result" "FAILED_OVERSELL_NEGATIVE_INVENTORY"
  exit 1
fi

if (( actual_consumption > inventory_before_real_race )); then
  err "Oversell real detectado: consumo real (${actual_consumption}) excede saldo inicial (${inventory_before_real_race})"
  update_env_value "ledger_race_result" "FAILED_OVERSELL_CONSUMPTION_EXCEEDED"
  exit 1
fi

if [[ "${actual_inventory_after}" != "${expected_inventory_after}" ]]; then
  err "Ledger mismatch: esperado ${expected_inventory_after}, atual ${actual_inventory_after} (baseline real=${inventory_before_real_race}, accepted_affecting=${accepted_affecting_count})"
...
if (( movements_delta < accepted_affecting_count )); then
  err "Movement delta inconsistente: delta=${movements_delta}, affecting_success_count=${accepted_affecting_count}, product=${PRODUCT_ID}"
...
ok "TESTE V22.2 ULTRA CORRIGIDA FINALIZADO"


hr
title "V22 - Observação"
detail "Esta suíte mantém 100% da V21 e adiciona novas requisições Postman na collection V22."
detail "Runner ultra corrigido com baseline real pré-chaos, reconciliação robusta e detecção automática de oversell."
