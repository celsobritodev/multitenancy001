...
# =========================================================
# TESTE V12 - VERSÃO ULTRA ROBUSTA (CORRIGIDA)
# =========================================================
# Características:
...
LOG_FILE="${LOG_DIR}/execucao_v12_${TIMESTAMP}.log"
APP_LOG="${LOG_DIR}/app_${TIMESTAMP}.log"
REPORT_DIR="${LOG_DIR}/reports_${TIMESTAMP}"
mkdir -p "${LOG_DIR}" "${REPORT_DIR}"

# Arquivos de teste
COLLECTION="${SCRIPT_DIR}/multitenancy001.postman_collection.v12.0.enterprise.json"
ENV_FILE="${SCRIPT_DIR}/multitenancy001.local.postman_environment.v8.0.json"
...
check_requirements() {
    ...
    # Script E2E não é mais necessário - removido da verificação
...
check_port() {
    ...
    # Verificar se a porta está em uso (funciona no Git Bash)
    if command -v netstat >/dev/null 2>&1; then
        if netstat -ano 2>/dev/null | grep -q ":${APP_PORT}.*LISTENING"; then
            ...
            # Obter PIDs dos processos usando a porta
            local pids
...
run_newman() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        # Extrair estatísticas se jq estiver disponível
...
extract_tokens() {
    ...
    # Extrair valores usando grep/sed (fallback se jq não estiver disponível)
    if command -v jq >/dev/null 2>&1; then
        local tenant_token=$(jq -r '.values[] | select(.key=="tenant_token") | .value' "${TEMP_ENV}" 2>/dev/null || echo "")
        ...
        local tenant_token=$(grep -o '"tenant_token":"[^"]*"' "${TEMP_ENV}" | head -1 | cut -d'"' -f4 || echo "")
    ...
    step "Token Superadmin: ${superadmin_token:0:20}... (${#superadmin_token} chars)"
...
generate_report() {
    ...
    {
        echo "╔════════════════════════════════════════════════════════════╗"
        echo "║           RELATÓRIO DE TESTES V12 - MULTITENANCY001       ║"
        echo "╚════════════════════════════════════════════════════════════╝"
        echo ""
...
main() {
    ...
    hr
    echo -e "${WHITE}🔷 TESTE V12 - MULTITENANCY001 (VERSÃO ULTRA ROBUSTA)${RESET}"
...
# Se houver argumentos, processar
case "${1:-}" in
    --help|-h)
        echo "Uso: $0 [OPÇÃO]"
        echo ""
