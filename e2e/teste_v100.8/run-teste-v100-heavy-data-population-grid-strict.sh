...
COLLECTION="${SCRIPT_DIR}/multitenancy001.postman_collection.v100.8.enterprise-data-population-grid.query-stress.strict.json"
ENV_FILE="${SCRIPT_DIR}/multitenancy001.local.postman_environment.v100.8.enterprise-data-population-grid.query-stress.strict.json"
...
trap cleanup EXIT
echo "────────────────────────────────────────────────────"
echo "🔷 TESTE V100.8 HEAVY DATA POPULATION GRID QUERY STRESS - STRICT SUITE"
echo "   Iniciando execução em: $(date)"
echo "   Path do script: ${SCRIPT_DIR}"
echo "────────────────────────────────────────────────────"
title "Verificando requisitos"
[[ -f "${COLLECTION}" ]] || COLLECTION="${ALT_COLLECTION}"
[[ -f "${ENV_FILE}" ]] || ENV_FILE="${ALT_ENV_FILE}"
[[ -f "${COLLECTION}" ]] || COLLECTION="${ALT_COLLECTION}"
