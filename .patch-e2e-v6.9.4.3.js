...
function makeCustomerItem(idx){
  ...
  return {
    ...
    request:{method:'POST',header:[
      {key:'Authorization',value:'Bearer {{tenant1_token}}',type:'text'},
      {key:'X-Tenant',value:'{{tenant1_schema}}',type:'text'},
      {key:'Content-Type',value:'application/json',type:'text'}
    ],body:{mode:'raw',raw:'{}'},url:{raw:'{{base_url}}/api/tenant/customers',host:['{{base_url}}'],path:['api','tenant','customers']}},
    response:[],
    event:[
      ...
      {listen:'test',script:{type:'text/javascript',exec:[
        "pm.test('status 2xx/201', function(){ pm.expect(pm.response.code).to.be.oneOf([200,201,204]); });","const res = pm.response.json();","const created = JSON.parse(pm.collectionVariables.get('created_customer_ids') || '[]');","if (res && res.id) { created.push(res.id); pm.collectionVariables.set('created_customer_ids', JSON.stringify(created)); }","console.log('✅ Customer criado com ID:', res && res.id);","console.log('🧾 Total de customers criados:', created.length);"
      ]}}
    ]
  };
}
