...
public class TenantPasswordResetSupport {
    ...
    public void assertResetTokenClaims(String tenantSchema, Long accountId, String email) {
        if (!StringUtils.hasText(tenantSchema)) {
            throw new ApiException(ApiErrorCode.INVALID_TOKEN, "Token inválido: tenantSchema ausente", 400);
        }
        if (accountId == null) {
            throw new ApiException(ApiErrorCode.INVALID_TOKEN, "Token inválido: accountId ausente", 400);
        }
        if (!StringUtils.hasText(email)) {
            throw new ApiException(ApiErrorCode.INVALID_TOKEN, "Token inválido: email ausente", 400);
        }
    }
}