...
public class AccountAppService {
    ...
    private void assertValidCreatedBetweenRange(Instant start, Instant end) {
        if (start == null || end == null) {
            throw new ApiException(ApiErrorCode.INVALID_RANGE, "start/end são obrigatórios");
        }
    ...
    public Page<Account> listAccountsByStatus(AccountStatus status, Pageable pageable) {
        if (status == null) {
            throw new ApiException(ApiErrorCode.INVALID_STATUS, "status é obrigatório");
        }
    ...
    public Page<Account> searchAccountsByDisplayName(String term, Pageable pageable) {
        if (term == null || term.isBlank()) {
            throw new ApiException(ApiErrorCode.INVALID_SEARCH, "term é obrigatório");
        }
        ...
        return publicSchemaUnitOfWork.readOnly(() ->
                accountRepository.searchByDisplayName(normalized, p)
        );
    }
